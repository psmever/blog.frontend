    ...
    <div className="space-y-8">
      <header className="space-y-3">
        ...
        <p className="text-sm text-foreground/70">
          API 연동 전까지는 샘플 데이터로 구성됩니다. 새 글이 준비되면 여기에서 바로 확인하실 수
          있어요.
        </p>
      </header>
      ...
      <Card className="border-dashed border-foreground/20 bg-muted/60">
        <CardHeader>
          ...
          <CardDescription>
            실제 게시글은 API 연동이 완료되는 대로 자동으로 표시됩니다.
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-2 text-sm text-foreground/75">
          <p>• 관리자는 로그인 후 새 글을 작성하고 공개 여부를 설정할 수 있습니다.</p>
          <p>
            • 초안 상태의 글은 `/posts` 목록에는 보이지 않으며, `/login` 경유 대시보드에서
            관리됩니다.
          </p>
          <p>• RSS, 태그/카테고리, 검색 기능은 API 스펙이 확정되면 함께 추가할 예정입니다.</p>
          <Link
            href="/login"
            className="inline-flex items-center font-semibold text-foreground/85 underline-offset-4 transition hover:text-foreground hover:underline"
          >
            관리자 로그인 바로가기 →
          </Link>
        </CardContent>
