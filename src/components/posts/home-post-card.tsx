  ...
  return (
    <Card className="group h-full overflow-hidden border border-foreground/10 bg-card shadow-sm transition hover:-translate-y-[2px] hover:border-foreground/25">
      <div className="relative aspect-[4/3] overflow-hidden bg-foreground/[0.04]">
        ...
        />
        {post.tag && (
          <span className="absolute left-3 top-3 rounded-full bg-background/85 px-3 py-1 text-xs font-semibold text-foreground shadow-sm">
            {post.tag}
          </span>
        )}
      </div>
      ...
      <CardContent className="space-y-3 p-4">
        ...
        <div className="flex items-center gap-3 text-xs text-foreground/60">
          <span>
            {new Intl.DateTimeFormat("ko", { dateStyle: "medium" }).format(new Date(post.date))}
          </span>
        ...
        <div className="flex items-center justify-between">
          ...
          <span
            className={cn(
              "rounded-full border border-foreground/10 px-3 py-1 text-xs font-semibold text-foreground/75 transition group-hover:border-foreground/30",
            )}
          >
            자세히 보기
          </span>
