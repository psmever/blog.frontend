...
export function PostList({ posts, emptyText }: PostListProps) {
    if (!posts.length) {
        return <div className="rounded-xl border border-dashed border-foreground/20 bg-background/80 p-6 text-sm text-foreground/70">{emptyText ?? "아직 게시글이 없습니다. 새로운 글을 준비 중이에요."}</div>;
    }
